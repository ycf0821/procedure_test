!>Description:
!>  This is a module adopted from Cowles's mod_newinp.F 
!>  and modified to facilitate input in the NHWAVE model.
!>
!>
!>private:
!>
...
!>
!>Subroutine:
!>  Get_Integer_Val(ival,filename,argname,line,echo,start,finish)
!>  Get_Float_Val(fval,filename,argname,line,echo,start,finish)
!>  Get_Logical_Val(lval,filename,argname,line,echo,start,finish)
!>  Get_String_Val(sval,filename,argname,line,ierr,echo,start,finish)
!>  Extract_Val_String(fname,argname,argval,ierr,line,start,finish)
!>  Check_Arg_Type(argval,argtype)
!>  Error(subname,errtype,err1,err2,err3)
!>  Check_Alloc(errorin,dsize,varname,subname)
!>  Check_IOerror(errorin,filename,varname)
!>
!>Reference:
!>
!>Author:
